...
import static pers.lzb.platform.Choice.choice;
...
public class SignUpAndLogIn {
    ...
    public static void signUp() {                  //注册
        ...
        Password = passage.nextLine();

        hashMap.put(Name, Password);     //将名字和密码添进哈希表
    ...
    public static int logIn() {                            //登录
        ...
        Scanner name = new Scanner(System.in);
        Name = name.nextLine();
        boolean temp = hashMap.containsKey(Name);
        if (!temp) {                                  //输入名字不正确或未注册
